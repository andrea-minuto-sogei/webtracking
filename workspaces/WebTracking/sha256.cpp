 ...
 * @param input message string used as an input to the SHA256 algorithm, must be
 * < size_t bits
 */
std::string SHA256::hash(const std::string &input) {
  ...
  uint32 *h = new uint32[HASH_LEN]; // buffer holding the message digest
                                    // (256-bit = 8 32-bit words)
...
uint32 **SHA256::preprocess(const unsigned char *input, size_t &nBuffer) {
  ...
  size_t k = (448 - 1 - l) % MESSAGE_BLOCK_SIZE; // length of zero bit padding
                                                 // (l + 1 + k = 448 mod 512)
