...
static int post_read_request(request_rec *r)
{
   ...
   if (r->prev)
   {
      if (APLOG_R_IS_LEVEL(r, APLOG_DEBUG)) ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "post_read_request(): [%ld] end (DECLINED)", tid);
      return DECLINED;
   }
   ...
   {
      if (APLOG_R_IS_LEVEL(r, APLOG_DEBUG)) ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "post_read_request(): [%ld] matched Trace URI = %s", tid, trace_uri_matched);
      trace_uri = 1;
   }
   ...
   if (APLOG_R_IS_LEVEL(r, APLOG_DEBUG)) ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "post_read_request(): [%ld] get or build uuid", tid);
